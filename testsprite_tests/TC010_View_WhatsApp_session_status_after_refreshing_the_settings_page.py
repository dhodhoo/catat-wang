...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email field with test.qa+20260421@catatwang.local, fill the password field with Test1234!@#, then submit the sign-in form and wait for the app to load.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the WhatsApp navigation link to open WhatsApp settings and wait for the session status to load.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/header/div/nav/a[6]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Reload the WhatsApp settings view (refresh) and confirm the session status section still displays the current connection state.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/header/div/nav/a[6]').nth(0)
        await asyncio.sleep(3); await elem.click()
