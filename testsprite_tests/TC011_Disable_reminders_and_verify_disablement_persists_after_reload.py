...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email field (index 278) with the test account email, then fill the password (index 282), then submit by clicking the Masuk button (index 285).
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'Pengingat' (Reminders) navigation link to open the reminders/settings page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/header/div/nav/a[5]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click 'Simpan pengaturan' to save the current (disabled) reminder setting, then navigate to Beranda and back to Pengingat to verify reminders remain disabled after reload/navigation.
        frame = context.pages[-1]
        # Click element
