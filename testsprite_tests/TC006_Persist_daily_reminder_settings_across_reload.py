...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields, then submit the sign-in form to authenticate the test user.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Open the 'Pengingat' (Reminders) page by clicking the navigation link, then locate the reminder enable toggle and schedule settings.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/header/div/nav/a[5]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the 'Nyalakan' (Enable) button to turn reminders on and wait for the UI to reflect the change.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[3]/main/div/section/div/div[2]/div/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click 'Simpan pengaturan' to save the reminders settings, reload the page, then verify that reminders remain enabled and the frequency/time persisted.
        frame = context.pages[-1]
        # Click element
